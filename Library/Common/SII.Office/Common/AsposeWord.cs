...
namespace Agro.Office
{
    ...
    public abstract class WordBase
    {
        ...
        public void Print(object data, bool close = true)
        {
            if (doc == null)
            {
        ...
        public void PrintPreview(object data)
        {
            try
            {
                ...
                bool printView = WordOperator.GetSpecialAppSettingValue("DocumentPrintvieMode", "false").ToLower() == "true";
                if (!printView)
                {
        ...
        public void SaveAs(object data, string fileName, bool close = true)
        {
            if (doc == null)
            {
                throw new Exception(TEMPLATEFILE_NOT_OPEN);
        ...
        /// <param name="data">数据</param>
        /// <param name="fileName">文件名称</param>
        public void SaveAsPdf(object data, string fileName)
        {
            if (doc == null)
            {
                throw new Exception(TEMPLATEFILE_NOT_OPEN);
            ...
            {
                fileName = extension == ".pdf" ? fileName : fileName.Replace(extension, ".pdf");
            }
            bool success = OnSetParamValue(data);
            if (success)
            {
        ...
        /// <param name="fileName">文件名称</param>
        /// <param name="close">是否关闭</param>
        public void SaveAsDocAndPdfFile(object data, string fileName)
        {
            if (doc == null)
            {
                throw new Exception(TEMPLATEFILE_NOT_OPEN);
        ...
        /// <param name="fileName">文件名称</param>
        /// <param name="close">是否关闭</param>
        public void SaveAsDocAndPdfAndJpgFile(object data, string fileName)
        {
            if (doc == null)
            {
                throw new Exception(TEMPLATEFILE_NOT_OPEN);
        ...
        /// <param name="fileName">文件名称</param>
        /// <param name="close">是否关闭</param>
        public void SaveAsJpgAndPdfFile(object data, string fileName)
        {
            if (doc == null)
            {
                throw new Exception(TEMPLATEFILE_NOT_OPEN);
        ...
        /// 保存为Jpg图片
        /// </summary>
        private void SaveJpepFile(string fileName)
        {
            ImageSaveOptions options = new ImageSaveOptions(SaveFormat.Jpeg);
            options.Resolution = 300;
            string nameFile = fileName + ".jpg";
            int number = doc.PageCount > 1 ? 1 : 0;
            for (int i = 0; i < doc.PageCount; i++)
            {
                nameFile = fileName + (number == 0 ? "" : number.ToString()) + ".jpg";
                options.PageIndex = i;
        ...
        /// <param name="paramName"></param>
        /// <param name="paramValue"></param>
        protected void SetCheckBoxValue(object paramName, object paramValue, int controlType = 71)
        {
